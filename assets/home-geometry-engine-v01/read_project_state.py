...
def state_gate(state: dict[str, Any]) -> dict[str, Any]:
    ...
    extraction_gate = state.get("base_source_extraction_gate")
    extraction_level = state.get("base_source_extraction_level") or base_level
    ...
    extraction_known = extraction_gate in {"passed", "warning", "failed"}
    ...
    extraction_quick_ok = (not extraction_known) or (extraction_gate in {"passed", "warning"} and level_at_least(extraction_level, "L2"))
    extraction_deepening_ok = (not extraction_known) or (extraction_gate == "passed" and level_at_least(extraction_level, "L3"))
    base_ok = base_status == "passed" and source_quick_ok and extraction_quick_ok and not blockers
    option_ok = option_status == "passed" and not blockers
    return {
        "can_quick_concept": base_ok and level_at_least(base_level, "L2"),
        "can_stable_deepening": base_ok and source_deepening_ok and extraction_deepening_ok and option_ok and level_at_least(option_level, "L3"),
        "base_level": base_level,
        "base_validation_status": base_status,
        "base_source_gate": source_gate or "unknown",
        "base_source_level": source_level,
        "base_source_extraction_gate": extraction_gate or "unknown",
        ...
        "reason": "ok" if base_ok and source_deepening_ok and extraction_deepening_ok and option_ok else "base_extraction_source_or_active_option_not_passed",
    }
...
def print_summary(state: dict[str, Any]) -> None:
    ...
    print(f"source_extraction: {gate['base_source_extraction_gate']} / {gate['base_source_extraction_level']}")
...
def main() -> int:
    ...
    if args.json:
        ...
        output = {
            ...
            "base_source_extraction_gate": gate["base_source_extraction_gate"],
            "base_source_extraction_level": gate["base_source_extraction_level"],

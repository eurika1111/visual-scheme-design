...
- `scheme-history`：登记方案版本、接受或淘汰版本、切换当前版本，以及从已接受版本创建安全分支。
- `build-visual-handoff`：从当前已接受方案生成结构锁、风格确认引导、视觉提示词和生图后检查清单。
- `render-scheme-draft`：从底图模型和方案意图生成确定性方案草图 SVG。
- `run-demo`：运行项目内完整示例流程。

底层脚本仍保留，但日常应优先走这个统一入口。

底图复核后的已确认修改使用 `import-staged-topology --corrections <json>` 传入。校正清单按稳定对象 ID 修改开口类型或补充房间显示扩展区；未知 ID 会阻止导入，报告会列出全部已应用校正。修改后必须重新导入、校验和渲染，不能直接编辑复核 SVG/PNG。
## 一键验证脚本
